...
if(isset($_POST['pat_submit']))
{
    ...
    if(isset($_POST['tra_submit']))
    {
        ...
        if(isset($_POST['pay_submit']))
        {
            ...
            $query="insert into fplan(fmod,source,destination,airspace,onboard)values('$Payment_id','$Amount','$customer_id','$payment_type','$customer_name')";
             $result=mysqli_query($con,$query);
            if($result)
            {
...
function get_payment(){
    global $con;
    $query="select * from fplan";
    $result=mysqli_query($con,$query);
    while($row=mysqli_fetch_array($result)){
        $Payment_id=$row ['fmod'];
        $Amount=$row['source'];
        $payment_type=$row['destination'];
        $customer_id=$row['airspace'];
        $customer_name=$row['onboard'];
